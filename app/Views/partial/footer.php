<link rel="stylesheet" href="css/style.css">
<footer class="footer-59391" style="background-color: whitesmoke;">
  <ul class="nav justify-content-center border-bottom pb-3 mb-3 ">
  <script>
    function scrollToBotto() {
        var divToScroll = document.getElementById('strong'); // Replace 'yourDivId' with the actual ID of your div
        divToScroll.scrollTop = divToScroll.scrollHeight;
    }
</script>

    <li class="nav-item"><a href="/" class="nav-link px-2 text-muted">Home</a></li>
    <li class="nav-item nav-link px-2 text-muted"><a href="#" onclick="scrollToBotto()">Features</a></li>
    <li class="nav-item"><a href="#" class="nav-link px-2 text-muted">Pricing</a></li>
    <li class="nav-item"><a href="#" class="nav-link px-2 text-muted">FAQs</a></li>
    <li class="nav-item"><a href="#" class="nav-link px-2 text-muted">About</a></li>
  </ul>
  <center><img src="<?= base_url('assets/image/logo.png') ?>" alt="" srcset="" class=""></center>
  <p class="text-center text-muted">© 2023 Gohit, Inc</p>
</footer>