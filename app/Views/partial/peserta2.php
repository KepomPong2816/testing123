                ...
                <div class="entry-player-pair-wrap">
                    <div class="entry-player-wrap">
                        <div class="entry-player-image">
                            <img src="https://w7.pngwing.com/pngs/529/972/png-transparent-award-prize-medal-computer-icons-award-culture-trophy-symbol-thumbnail.png"
                                class=" b-error">
                        </div>
...
<form id="join-form" method="POST" action="/join/add">
    <input type="hidden" name="idevent" value="<?php echo $selectedevent[0]['id_event'] ?>"
        class="formbold-form-input" />
    <center>
        <?php if (isset($show_join_button)) { ?>
            <?php if (isset($nama)) { ?>
                <button type="button" class="btn btn-primary fw-bold" style="margin-top: 2cm;"
                    onclick="showConfirmation()">JOIN</button>
            <?php } else { ?>
                <button type="button" class="btn btn-primary fw-bold" style="margin-top: 2cm;" onclick="login()">JOIN</button>
            <?php } ?>
        <?php } else { ?>
            <?php if (isset($nama)) { ?>
                <button type="button" class="btn btn-primary fw-bold" style="margin-top: 2cm;"
